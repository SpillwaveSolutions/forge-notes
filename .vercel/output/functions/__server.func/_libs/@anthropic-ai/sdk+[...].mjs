...
*/
var RetryableError = class extends AnthropicError {
	constructor(message, { cause } = {}) {
...
//#endregion
//#region node_modules/@anthropic-ai/sdk/internal/request-signal.mjs
/**
...
//#endregion
//#region node_modules/@anthropic-ai/sdk/internal/qs/formats.mjs
...
//#endregion
//#region node_modules/@anthropic-ai/sdk/internal/qs/utils.mjs
...
//#endregion
//#region node_modules/@anthropic-ai/sdk/internal/qs/stringify.mjs
...
//#endregion
//#region node_modules/@anthropic-ai/sdk/lib/credentials/types.mjs
...
function requireSecureTokenEndpoint(baseURL) {
	...
	if (u.protocol === "http:" && (host === "localhost" || host === "127.0.0.1" || host === "::1")) return;
...
async function parseTokenResponse(resp, requestId) {
	...
	if (!data.access_token) throw new WorkloadIdentityError(`Token endpoint response missing access_token: ${JSON.stringify(redactSensitive(data))}`, resp.status, redactSensitive(data), requestId);
	if (data.token_type && data.token_type.toLowerCase() !== "bearer") throw new WorkloadIdentityError(`Token endpoint response: unsupported token_type "${data.token_type}" (want Bearer)`, resp.status, redactSensitive(data), requestId);
	return data;
}
var MAX_ERROR_BODY_CHARS = 2e3;
var SAFE_ERROR_KEYS = /* @__PURE__ */ new Set([
	"error",
	"error_description",
	"error_uri"
]);
...
function redactSensitive(body) {
	...
	if (typeof body === "object" && !Array.isArray(body)) {
		const out = {};
		for (const [k, v] of Object.entries(body)) if (SAFE_ERROR_KEYS.has(k)) out[k] = v;
		return out;
	}
	return null;
}
...
async function checkCredentialsFileSafety(path, onWarn = (m) => console.warn(`anthropic-sdk: ${m}`)) {
	...
	if (mode & 18) throw new WorkloadIdentityError(`Credentials file at ${resolved} is group/world-writable (mode 0o${mode.toString(8)}); this allows other local users to plant tokens. Run \`chmod 600 ${resolved}\`.`);
	if (mode & 36) throw new WorkloadIdentityError(`Credentials file at ${resolved} is group/world-readable (mode 0o${mode.toString(8)}); run \`chmod 600 ${resolved}\` before retrying.`);
	if (typeof process.getuid === "function" && st.uid !== process.getuid()) onWarn(`credentials file at ${resolved} is owned by uid ${st.uid} (current process uid ${process.getuid()}); verify this is intentional.`);
}
/**
...
async function writeCredentialsFileAtomic(targetPath, data) {
	const fs = await import("node:fs");
	const dir = (await import("node:path")).dirname(targetPath);
...
//#endregion
//#region node_modules/@anthropic-ai/sdk/internal/utils/time.mjs
/** Current time as unix epoch seconds. */
function nowAsSeconds() {
	return Math.floor(Date.now() / 1e3);
}
//#endregion
//#region node_modules/@anthropic-ai/sdk/lib/credentials/token-cache.mjs
/**
...
var TokenCache = class {
	...
	async getToken() {
		...
		if (remaining > 120) return cached.token;
		if (remaining > 30) {
	...
	*/
	backgroundRefresh() {
		...
		if (nowAsSeconds() - this.lastAdvisoryError < 5) return;
...
//#endregion
//#region node_modules/@anthropic-ai/sdk/core/credentials.mjs
var PROFILE_NAME_PATTERN = /^[A-Za-z0-9_.-]+$/;
function validateProfileName(name) {
	if (!name) throw new Error("profile name is empty");
	if (name === "." || name === "..") throw new Error(`profile name "${name}" is not allowed`);
	if (name.includes("/") || name.includes("\\")) throw new Error(`profile name "${name}" must not contain path separators`);
	if (!PROFILE_NAME_PATTERN.test(name)) throw new Error(`profile name "${name}" contains disallowed characters (allowed: letters, digits, '_', '.', '-')`);
}
/**
...
var loadConfigWithSource = async (profile) => {
	...
	if (rootConfigPath === null) return null;
	const profileName = profile ?? await getActiveProfileName();
	if (profileName === null) return null;
	validateProfileName(profileName);
	const fs = await import("node:fs");
	const configPath = (await import("node:path")).join(rootConfigPath, "configs", `${profileName}.json`);
	let configRaw;
	try {
		configRaw = await fs.promises.readFile(configPath, "utf-8");
	} catch (err) {
		if (err?.code !== "ENOENT") throw new Error(`failed to read config file ${configPath}: ${err}`);
		configRaw = null;
	}
	if (configRaw === null) {
	...
	if (!config.authentication) throw new Error(`config file ${configPath} is missing "authentication"`);
	const authType = config.authentication.type;
	if (authType !== "oidc_federation" && authType !== "user_oauth") throw new Error(`authentication.type "${authType}" is not a known authentication type`);
	...
	if (config.authentication.type === "oidc_federation") {
		...
		if (!config.authentication.federation_rule_id) config.authentication.federation_rule_id = readEnv("ANTHROPIC_FEDERATION_RULE_ID") ?? "";
...
var getCredentialsPath = async (config, profile) => {
	...
	validateProfileName(profileName);
	return (await import("node:path")).join(rootConfigPath, "credentials", `${profileName}.json`);
};
var getRootConfigPath = async () => {
	if (!supportsLocalConfigFiles()) return null;
	const path = await import("node:path");
	const configDir = readEnv("ANTHROPIC_CONFIG_DIR");
	if (configDir) return configDir;
	if (getPlatformHeaders()["X-Stainless-OS"] === "Windows") {
...
var getActiveProfileName = async () => {
	...
	const fs = await import("node:fs");
	const filePath = (await import("node:path")).join(rootConfigPath, "active_config");
	try {
		return (await fs.promises.readFile(filePath, "utf-8")).trim() || "default";
	} catch (err) {
		if (err?.code !== "ENOENT") throw new Error(`failed to read ${filePath}: ${err}`);
		return "default";
	}
};
//#endregion
//#region node_modules/@anthropic-ai/sdk/lib/credentials/identity-token.mjs
/**
...
function identityTokenFromFile(path) {
	if (!path) throw new AnthropicError("Identity token file path is empty");
	return async () => {
		const fs = await import("node:fs");
		let content;
		try {
		...
		if (!token) throw new AnthropicError(`Identity token file at ${path} is empty`);
		return token;
	};
}
/**
...
function identityTokenFromValue(token) {
	if (!token) throw new AnthropicError("Identity token value is empty");
	return () => token;
}
//#endregion
//#region node_modules/@anthropic-ai/sdk/lib/credentials/oidc-federation.mjs
/**
...
function oidcFederationProvider(config) {
	return async () => {
		...
		if (jwt.length > 16 * 1024) throw new WorkloadIdentityError(`Identity token is ${Math.ceil(jwt.length / 1024)} KiB, exceeds the 16 KiB assertion limit`);
		const body = {
		...
		if (config.serviceAccountId) body["service_account_id"] = config.serviceAccountId;
		...
		try {
			resp = await config.fetch(url, {
				...
				headers: {
					...
					"User-Agent": config.userAgent || `anthropic-sdk-typescript/0.115.0 oidcFederationProvider`
				},
				body: JSON.stringify(body)
			});
		} catch (err) {
		...
		if (!resp.ok) {
			const redacted = redactSensitive(await resp.text().catch(() => ""));
			let hint = "";
			if (resp.status === 401) hint = ` Ensure your federation rule matches your identity token. ${config.workspaceId ? "" : "If your federation rule is scoped to multiple workspaces, set the ANTHROPIC_WORKSPACE_ID environment variable, the 'workspace_id' config key, or the `workspaceId` option. "}View your authentication events in the Workload identity page of Claude Console for more details.`;
		...
		if (!Number.isFinite(expiresIn)) throw new WorkloadIdentityError(`Token endpoint response missing required fields: ${JSON.stringify(redactSensitive(data))}`, resp.status, redactSensitive(data), requestId);
		return {
...
//#endregion
//#region node_modules/@anthropic-ai/sdk/lib/credentials/user-oauth.mjs
/**
...
function userOAuthProvider(config) {
	return async (opts) => {
		...
		if (!accessToken) throw new WorkloadIdentityError(`Credentials file at ${config.credentialsPath} must include 'access_token'`);
		const expiresAt = creds.expires_at;
		if (!opts?.forceRefresh && (expiresAt == null || nowAsSeconds() < expiresAt - 30)) return {
			token: accessToken,
			expiresAt: expiresAt ?? null
		};
		const refreshToken = creds.refresh_token;
		if (!config.clientId || !refreshToken) throw new WorkloadIdentityError(`Access token at ${config.credentialsPath} has expired and no refresh is available (client_id ${config.clientId ? "set" : "empty"}, refresh_token ${refreshToken ? "set" : "empty"})`);
		requireSecureTokenEndpoint(config.baseURL);
		const body = {
		...
		try {
			resp = await config.fetch(url, {
				...
				headers: {
					...
					"User-Agent": config.userAgent || `anthropic-sdk-typescript/0.115.0 userOAuthProvider`
				},
				body: JSON.stringify(body)
			});
		} catch (err) {
		...
		const data = await parseTokenResponse(resp, requestId);
		const expiresIn = Number(data.expires_in);
		if (!Number.isFinite(expiresIn)) throw new WorkloadIdentityError(`User OAuth refresh response missing or invalid expires_in: ${JSON.stringify(redactSensitive(data))}`, resp.status, redactSensitive(data), requestId);
...
//#endregion
//#region node_modules/@anthropic-ai/sdk/lib/credentials/credential-chain.mjs
function resolveCredentialsFromConfig(config, options) {
	const provider = buildProvider(config, config.authentication.credentials_path ?? null, (config.base_url || options.baseURL).replace(/\/+$/, ""), options);
	const extraHeaders = {};
	if (config.workspace_id && config.authentication.type === "user_oauth") extraHeaders["anthropic-workspace-id"] = config.workspace_id;
	return {
		provider,
		extraHeaders,
		baseURL: config.base_url || void 0
	};
}
/**
...
async function defaultCredentials(options, profile) {
	...
	return resolveCredentialsFromConfig(config.authentication.credentials_path || !fromFile ? config : {
		...config,
		authentication: {
...
function buildProvider(config, credentialsPath, baseURL, options) {
	switch (config.authentication.type) {
		case "oidc_federation": {
			...
			if (!identityProvider) throw new WorkloadIdentityError("oidc_federation config requires an identity token (set authentication.identity_token, ANTHROPIC_IDENTITY_TOKEN_FILE, or ANTHROPIC_IDENTITY_TOKEN)");
			if (!auth.federation_rule_id) throw new WorkloadIdentityError("oidc_federation config requires 'federation_rule_id'. Set it in authentication.federation_rule_id in your profile, or via ANTHROPIC_FEDERATION_RULE_ID (profile takes precedence).");
			if (!config.organization_id) throw new WorkloadIdentityError("oidc_federation config requires organization_id (set ANTHROPIC_ORGANIZATION_ID or config.organization_id)");
			...
			if (credentialsPath) return cachedExchangeProvider(exchange, credentialsPath, options.onCacheWriteError, options.onSafetyWarning);
			return exchange;
		}
		case "user_oauth":
			if (!credentialsPath) throw new WorkloadIdentityError("user_oauth config requires authentication.credentials_path (or load via a profile so it defaults to <config_dir>/credentials/<profile>.json)");
...
function resolveIdentityTokenProvider(auth) {
	if (auth.identity_token) {
		...
		if (source !== "file") throw new WorkloadIdentityError(`identity_token.source "${source}" is not supported by this SDK version (only "file")`);
		if (!auth.identity_token.path) throw new WorkloadIdentityError(`identity_token.source "file" requires a non-empty path`);
		return identityTokenFromFile(auth.identity_token.path);
	}
	const tokenFile = readEnv("ANTHROPIC_IDENTITY_TOKEN_FILE");
	if (tokenFile) return identityTokenFromFile(tokenFile);
	const tokenValue = readEnv("ANTHROPIC_IDENTITY_TOKEN");
	if (tokenValue) return identityTokenFromValue(tokenValue);
	return null;
}
/**
	...
	return async (opts) => {
		...
		try {
			...
			if (token && !opts?.forceRefresh) {
				...
				if (expiresAt == null || nowAsSeconds() < expiresAt - 30) return {
					token,
					expiresAt: expiresAt ?? null
				};
			}
		} catch (err) {
			if (err?.code !== "ENOENT" && !(err instanceof SyntaxError)) onCacheWriteError?.(err);
		}
...
async function defaultParseResponse(client, props) {
	...
	const body = await (async () => {
		...
		if (!props.options.stream && !props.options.__binaryResponse) releaseRequestSignal(props.controller);
	});
...
//#endregion
//#region node_modules/@anthropic-ai/sdk/core/middleware.mjs
/**
...
function isRetryableError(err) {
	...
	while (typeof err === "object" && err !== null && !seen.has(err)) {
		seen.add(err);
		if (isFetchOriginError(err) || isAbortError(err) || err instanceof APIConnectionError || err instanceof RetryableError) return true;
		err = err.cause;
	}
	return false;
}
...
function wrapFetchWithMiddleware(fetchFn, middleware, options, client) {
	return async (url, init = {}) => {
		if (middleware.length === 0) return fetchFn.call(void 0, url, init);
		...
		if (response.bodyUsed || response.body?.locked) throw new AnthropicError("middleware consumed the response body; use response.clone() to inspect it, or return new Response(body, response) to consume and replace it");
		return response;
	};
}
/**
...
function createMiddlewareContext(options, client) {
	...
	return {
		...
		parse(response) {
			if (options?.stream && response.ok) return parseMiddlewareResponse(response, options);
...
async function parseMiddlewareResponse(response, options) {
	if (response.bodyUsed || response.body?.locked) throw new AnthropicError("cannot ctx.parse() a response whose body was already consumed; call ctx.parse() instead of reading the body, or read via response.clone()");
	if (options?.stream && response.ok) return Stream.fromSSEResponse(response.clone(), new AbortController());
	if (response.status === 204) return null;
	if (options?.__binaryResponse) return response;
	const mediaType = response.headers.get("content-type")?.split(";")[0]?.trim();
	if (mediaType?.includes("application/json") || mediaType?.endsWith("+json")) {
		if (response.headers.get("content-length") === "0") return;
...
var appendHeaderValue = (existing, addition) => {
	...
	for (const tok of addition.split(",").map((t) => t.trim())) if (tok && !tokens.includes(tok)) tokens.push(tok);
	return tokens.join(", ");
};
...
//#endregion
//#region node_modules/@anthropic-ai/sdk/resources/beta/deployment-runs.mjs
var DeploymentRuns = class extends APIResource {
	...
	*/
	retrieve(deploymentRunID, params = {}, options) {
		const { betas } = params ?? {};
		return this._client.get(path$2`/v1/deployment_runs/${deploymentRunID}?beta=true`, {
			...options,
...
//#endregion
//#region node_modules/@anthropic-ai/sdk/resources/beta/deployments.mjs
var Deployments = class extends APIResource {
	...
	*/
	retrieve(deploymentID, params = {}, options) {
		const { betas } = params ?? {};
		return this._client.get(path$2`/v1/deployments/${deploymentID}?beta=true`, {
			...options,
			headers: buildHeaders([{ "anthropic-beta": [...betas ?? [], "managed-agents-2026-04-01"].toString() }, options?.headers])
		});
	}
	...
	*/
	update(deploymentID, params, options) {
		const { betas, ...body } = params;
		return this._client.post(path$2`/v1/deployments/${deploymentID}?beta=true`, {
			body,
			...options,
			headers: buildHeaders([{ "anthropic-beta": [...betas ?? [], "managed-agents-2026-04-01"].toString() }, options?.headers])
		});
	...
	*/
	archive(deploymentID, params = {}, options) {
		const { betas } = params ?? {};
		return this._client.post(path$2`/v1/deployments/${deploymentID}/archive?beta=true`, {
			...options,
			headers: buildHeaders([{ "anthropic-beta": [...betas ?? [], "managed-agents-2026-04-01"].toString() }, options?.headers])
		});
	}
	...
	*/
	pause(deploymentID, params = {}, options) {
		const { betas } = params ?? {};
		return this._client.post(path$2`/v1/deployments/${deploymentID}/pause?beta=true`, {
			...options,
			headers: buildHeaders([{ "anthropic-beta": [...betas ?? [], "managed-agents-2026-04-01"].toString() }, options?.headers])
		});
	}
	...
	*/
	run(deploymentID, params = {}, options) {
		const { betas } = params ?? {};
		return this._client.post(path$2`/v1/deployments/${deploymentID}/run?beta=true`, {
			...options,
			headers: buildHeaders([{ "anthropic-beta": [...betas ?? [], "managed-agents-2026-04-01"].toString() }, options?.headers])
		});
	}
	...
	*/
	unpause(deploymentID, params = {}, options) {
		const { betas } = params ?? {};
		return this._client.post(path$2`/v1/deployments/${deploymentID}/unpause?beta=true`, {
			...options,
			headers: buildHeaders([{ "anthropic-beta": [...betas ?? [], "managed-agents-2026-04-01"].toString() }, options?.headers])
		});
	}
...
//#endregion
//#region node_modules/@anthropic-ai/sdk/resources/beta/dreams.mjs
var Dreams = class extends APIResource {
	...
	*/
	retrieve(dreamID, params = {}, options) {
		const { betas } = params ?? {};
		return this._client.get(path$2`/v1/dreams/${dreamID}?beta=true`, {
			...options,
			headers: buildHeaders([{ "anthropic-beta": [...betas ?? [], "dreaming-2026-04-21"].toString() }, options?.headers])
		});
	}
	...
	*/
	archive(dreamID, params = {}, options) {
		const { betas } = params ?? {};
		return this._client.post(path$2`/v1/dreams/${dreamID}/archive?beta=true`, {
			...options,
			headers: buildHeaders([{ "anthropic-beta": [...betas ?? [], "dreaming-2026-04-21"].toString() }, options?.headers])
		});
	}
	...
	*/
	cancel(dreamID, params = {}, options) {
		const { betas } = params ?? {};
		return this._client.post(path$2`/v1/dreams/${dreamID}/cancel?beta=true`, {
			...options,
			headers: buildHeaders([{ "anthropic-beta": [...betas ?? [], "dreaming-2026-04-21"].toString() }, options?.headers])
		});
	}
...
//#endregion
//#region node_modules/@anthropic-ai/sdk/internal/stainless-helper-header.mjs
/**
...
function collectStainlessHelpers(tools, messages) {
	...
	if (messages) for (const message of messages) {
		...
		if (Array.isArray(content)) {
			for (const block of content) if (wasCreatedByStainlessHelper(block)) helpers.add(block[SDK_HELPER_SYMBOL]);
		}
	}
...
function stainlessHelperHeaderFromFile(file) {
	if (wasCreatedByStainlessHelper(file)) return { [STAINLESS_HELPER_HEADER]: file[SDK_HELPER_SYMBOL] };
	return {};
}
//#endregion
...
var Files = class extends APIResource {
	...
	*/
	delete(fileID, params = {}, options) {
		const { betas } = params ?? {};
		return this._client.delete(path$2`/v1/files/${fileID}?beta=true`, {
			...options,
			headers: buildHeaders([{ "anthropic-beta": [...betas ?? [], "files-api-2025-04-14"].toString() }, options?.headers])
		});
	}
	...
	*/
	download(fileID, params = {}, options) {
		const { betas } = params ?? {};
		return this._client.get(path$2`/v1/files/${fileID}/content?beta=true`, {
			...options,
	...
	*/
	retrieveMetadata(fileID, params = {}, options) {
		const { betas } = params ?? {};
		return this._client.get(path$2`/v1/files/${fileID}?beta=true`, {
			...options,
			headers: buildHeaders([{ "anthropic-beta": [...betas ?? [], "files-api-2025-04-14"].toString() }, options?.headers])
		});
	}
...
var Models$1 = class extends APIResource {
	...
	*/
	retrieve(modelID, params = {}, options) {
		const { betas } = params ?? {};
		return this._client.get(path$2`/v1/models/${modelID}?beta=true`, {
			...options,
...
//#endregion
//#region node_modules/@anthropic-ai/sdk/resources/beta/user-profiles.mjs
var UserProfiles = class extends APIResource {
	...
	*/
	retrieve(userProfileID, params = {}, options) {
		const { betas } = params ?? {};
		return this._client.get(path$2`/v1/user_profiles/${userProfileID}?beta=true`, {
			...options,
			headers: buildHeaders([{ "anthropic-beta": [...betas ?? [], "user-profiles-2026-03-24"].toString() }, options?.headers])
		});
	}
	...
	*/
	update(userProfileID, params, options) {
		const { betas, ...body } = params;
		return this._client.post(path$2`/v1/user_profiles/${userProfileID}?beta=true`, {
			body,
			...options,
			headers: buildHeaders([{ "anthropic-beta": [...betas ?? [], "user-profiles-2026-03-24"].toString() }, options?.headers])
		});
	...
	*/
	createEnrollmentURL(userProfileID, params = {}, options) {
		const { betas } = params ?? {};
		return this._client.post(path$2`/v1/user_profiles/${userProfileID}/enrollment_url?beta=true`, {
			...options,
			headers: buildHeaders([{ "anthropic-beta": [...betas ?? [], "user-profiles-2026-03-24"].toString() }, options?.headers])
		});
	}
...
//#endregion
//#region node_modules/@anthropic-ai/sdk/resources/beta/webhooks.mjs
...
//#endregion
//#region node_modules/@anthropic-ai/sdk/resources/beta/agents/versions.mjs
var Versions$1 = class extends APIResource {
	...
	*/
	list(agentID, params = {}, options) {
		const { betas, ...query } = params ?? {};
		return this._client.getAPIList(path$2`/v1/agents/${agentID}/versions?beta=true`, PageCursor, {
			query,
			...options,
			headers: buildHeaders([{ "anthropic-beta": [...betas ?? [], "managed-agents-2026-04-01"].toString() }, options?.headers])
		});
...
//#endregion
//#region node_modules/@anthropic-ai/sdk/resources/beta/agents/agents.mjs
var Agents = class extends APIResource {
	...
	*/
	retrieve(agentID, params = {}, options) {
		const { betas, ...query } = params ?? {};
		return this._client.get(path$2`/v1/agents/${agentID}?beta=true`, {
			query,
			...options,
			headers: buildHeaders([{ "anthropic-beta": [...betas ?? [], "managed-agents-2026-04-01"].toString() }, options?.headers])
		});
	...
	*/
	update(agentID, params, options) {
		const { betas, ...body } = params;
		return this._client.post(path$2`/v1/agents/${agentID}?beta=true`, {
			body,
			...options,
			headers: buildHeaders([{ "anthropic-beta": [...betas ?? [], "managed-agents-2026-04-01"].toString() }, options?.headers])
		});
	...
	*/
	archive(agentID, params = {}, options) {
		const { betas } = params ?? {};
		return this._client.post(path$2`/v1/agents/${agentID}/archive?beta=true`, {
			...options,
			headers: buildHeaders([{ "anthropic-beta": [...betas ?? [], "managed-agents-2026-04-01"].toString() }, options?.headers])
		});
	}
...
//#endregion
//#region node_modules/@anthropic-ai/sdk/internal/utils/abort.mjs
/**
...
//#endregion
//#region node_modules/@anthropic-ai/sdk/internal/utils/backoff.mjs
...
function applyJitter(ms) {
	return ms * (1 - Math.random() * .25);
}
//#endregion
//#region node_modules/@anthropic-ai/sdk/lib/helper-client.mjs
/**
...
function copyClientForHelper(client, { authToken, helper }) {
	if (!authToken) throw new AnthropicError(`copyClientForHelper: expected a non-empty authToken but received ${JSON.stringify(authToken)}`);
	...
	const defaultHeaders = buildHeaders([
		parentAuthExtraHeaders ? Object.fromEntries(Object.entries(parentAuthExtraHeaders).filter(([name]) => {
...
//#endregion
//#region node_modules/@anthropic-ai/sdk/lib/environments/poller.mjs
...
var WorkPoller = class {
	constructor(opts) {
		...
		__classPrivateFieldSet(this, _WorkPoller_blockMs, opts.blockMs === void 0 ? 999 : opts.blockMs, "f");
	...
	async *[(_WorkPoller_runnerClient = /* @__PURE__ */ new WeakMap(), _WorkPoller_consumed = /* @__PURE__ */ new WeakMap(), _WorkPoller_controller = /* @__PURE__ */ new WeakMap(), _WorkPoller_detachExternal = /* @__PURE__ */ new WeakMap(), _WorkPoller_autoStop = /* @__PURE__ */ new WeakMap(), _WorkPoller_drain = /* @__PURE__ */ new WeakMap(), _WorkPoller_blockMs = /* @__PURE__ */ new WeakMap(), _WorkPoller_reclaimOlderThanMs = /* @__PURE__ */ new WeakMap(), _WorkPoller_requestOpts = /* @__PURE__ */ new WeakMap(), Symbol.asyncIterator)]() {
		if (__classPrivateFieldGet(this, _WorkPoller_consumed, "f")) throw new AnthropicError("Cannot iterate over a consumed WorkPoller");
			...
			while (!__classPrivateFieldGet(this, _WorkPoller_controller, "f").signal.aborted) {
				...
				try {
					work = await __classPrivateFieldGet(this, _WorkPoller_runnerClient, "f").beta.environments.work.poll(this.environmentId, {
						...
						headers: buildHeaders([__classPrivateFieldGet(this, _WorkPoller_requestOpts, "f")?.headers]),
						signal: __classPrivateFieldGet(this, _WorkPoller_controller, "f").signal
					});
				} catch (e) {
					if (__classPrivateFieldGet(this, _WorkPoller_controller, "f").signal.aborted) return;
					if (isFatal4xx(e)) {
				...
				attempt = 0;
				if (work == null) {
					if (__classPrivateFieldGet(this, _WorkPoller_drain, "f")) return;
				...
				try {
					await __classPrivateFieldGet(this, _WorkPoller_runnerClient, "f").beta.environments.work.ack(work.id, { environment_id: work.environment_id }, {
						headers: buildHeaders([__classPrivateFieldGet(this, _WorkPoller_requestOpts, "f")?.headers]),
						signal: __classPrivateFieldGet(this, _WorkPoller_controller, "f").signal
					});
				} catch (e) {
				...
				try {
					...
					if (__classPrivateFieldGet(this, _WorkPoller_autoStop, "f")) try {
						...
						if (!isStatus(e, 409)) log.warn("stop failed", {
							work_id: work.id,
							error: String(e)
						});
					}
...
function defaultWorkerId() {
	const host = (globalThis.process?.env)?.["HOSTNAME"];
	return host ? `${host}-${uuid4()}` : uuid4();
}
//#endregion
//#region node_modules/@anthropic-ai/sdk/internal/utils/async-queue.mjs
...
var AsyncQueue = class {
	...
	push(item) {
		if (__classPrivateFieldGet(this, _AsyncQueue_closed, "f")) return false;
		const w = __classPrivateFieldGet(this, _AsyncQueue_waiters, "f").shift();
		if (w) w({
			done: false,
			value: item
		});
		else __classPrivateFieldGet(this, _AsyncQueue_items, "f").push(item);
		return true;
	}
	/** Mark the queue done. Idempotent; wakes every pending reader with `done: true`. */
	close() {
		if (__classPrivateFieldGet(this, _AsyncQueue_closed, "f")) return;
		__classPrivateFieldSet(this, _AsyncQueue_closed, true, "f");
		while (__classPrivateFieldGet(this, _AsyncQueue_waiters, "f").length > 0) __classPrivateFieldGet(this, _AsyncQueue_waiters, "f").shift()({
			done: true,
			value: void 0
		});
	}
	...
	*/
	next(signal) {
		if (__classPrivateFieldGet(this, _AsyncQueue_items, "f").length > 0) return Promise.resolve({
			done: false,
			value: __classPrivateFieldGet(this, _AsyncQueue_items, "f").shift()
		});
		if (__classPrivateFieldGet(this, _AsyncQueue_closed, "f") || signal?.aborted) return Promise.resolve({
			done: true,
			value: void 0
		});
		return new Promise((resolve) => {
			...
			const onAbort = () => {
				...
				if (idx >= 0) __classPrivateFieldGet(this, _AsyncQueue_waiters, "f").splice(idx, 1);
				resolve({
					done: true,
					value: void 0
				});
...
//#endregion
//#region node_modules/@anthropic-ai/sdk/lib/tools/BetaRunnableTool.mjs
/**
...
async function runRunnableTool(tool, rawInput, context) {
	try {
		...
		return {
			content: await tool.run(input, context),
			isError: false
		};
	} catch (e) {
		return {
			content: toolErrorContent(e),
			isError: true
		};
	}
}
//#endregion
//#region node_modules/@anthropic-ai/sdk/lib/tools/SessionToolRunner.mjs
...
var IdleClock = class {
	...
	*/
	arm() {
		...
		if (__classPrivateFieldGet(this, _IdleClock_timer, "f") !== void 0) clearTimeout(__classPrivateFieldGet(this, _IdleClock_timer, "f"));
...
var SessionToolRunner = class {
	constructor(sessionId, opts) {
		...
		this.maxIdleMs = opts.maxIdleMs ?? 6e4;
	...
	async *[(_SessionToolRunner_consumed = /* @__PURE__ */ new WeakMap(), _SessionToolRunner_controller = /* @__PURE__ */ new WeakMap(), _SessionToolRunner_detachExternal = /* @__PURE__ */ new WeakMap(), _SessionToolRunner_requestOpts = /* @__PURE__ */ new WeakMap(), _SessionToolRunner_toolByName = /* @__PURE__ */ new WeakMap(), _SessionToolRunner_logger = /* @__PURE__ */ new WeakMap(), _SessionToolRunner_seen = /* @__PURE__ */ new WeakMap(), _SessionToolRunner_answered = /* @__PURE__ */ new WeakMap(), _SessionToolRunner_confirmationVerdicts = /* @__PURE__ */ new WeakMap(), _SessionToolRunner_awaitingConfirmation = /* @__PURE__ */ new WeakMap(), _SessionToolRunner_results = /* @__PURE__ */ new WeakMap(), _SessionToolRunner_inFlightCount = /* @__PURE__ */ new WeakMap(), _SessionToolRunner_onIdle = /* @__PURE__ */ new WeakMap(), _SessionToolRunner_idleClock = /* @__PURE__ */ new WeakMap(), _SessionToolRunner_instances = /* @__PURE__ */ new WeakSet(), Symbol.asyncIterator)]() {
		if (__classPrivateFieldGet(this, _SessionToolRunner_consumed, "f")) throw new AnthropicError("Cannot iterate over a consumed SessionToolRunner");
		...
		const streamPromise = __classPrivateFieldGet(this, _SessionToolRunner_instances, "m", _SessionToolRunner_streamLoop).call(this).catch((e) => {
			if (!__classPrivateFieldGet(this, _SessionToolRunner_controller, "f").signal.aborted) __classPrivateFieldGet(this, _SessionToolRunner_logger, "f").error("stream loop failed", { error: String(e) });
			__classPrivateFieldGet(this, _SessionToolRunner_controller, "f").abort();
		});
		try {
			while (true) {
			...
			await streamPromise;
			let pending;
			while ((pending = __classPrivateFieldGet(this, _SessionToolRunner_results, "f").tryShift()) !== void 0) yield pending;
		} finally {
			__classPrivateFieldGet(this, _SessionToolRunner_controller, "f").abort();
			...
			for (const t of this.tools) try {
				await t.close?.();
			} catch (e) {
				__classPrivateFieldGet(this, _SessionToolRunner_logger, "f").warn("tool.close failed", {
					tool: toolName(t),
					error: String(e)
				});
			}
...
_SessionToolRunner_requestOptions = function _SessionToolRunner_requestOptions() {
	...
	while (!ctrl.signal.aborted) {
		try {
			...
			for await (const ev of stream) {
				...
				if (await __classPrivateFieldGet(this, _SessionToolRunner_instances, "m", _SessionToolRunner_handleStreamEvent).call(this, ev)) return;
			}
		} catch (e) {
...
}, _SessionToolRunner_reconcile = async function _SessionToolRunner_reconcile() {
	const ctrl = __classPrivateFieldGet(this, _SessionToolRunner_controller, "f");
	const pending = [];
	...
	try {
		...
		for (const ev of pending) __classPrivateFieldGet(this, _SessionToolRunner_seen, "f").delete(ev.id);
		return;
	}
	...
	for (const held of [...__classPrivateFieldGet(this, _SessionToolRunner_awaitingConfirmation, "f").values()]) {
		...
		if (verdict !== void 0) await __classPrivateFieldGet(this, _SessionToolRunner_instances, "m", _SessionToolRunner_applyVerdict).call(this, held, verdict);
	}
	...
	if (ev.type === "agent.tool_use" || ev.type === "agent.custom_tool_use") {
		...
		if (!__classPrivateFieldGet(this, _SessionToolRunner_answered, "f").has(ev.id)) pending.push(ev);
	} else if (ev.type === "user.tool_result") __classPrivateFieldGet(this, _SessionToolRunner_answered, "f").add(ev.tool_use_id);
	else if (ev.type === "user.custom_tool_result") __classPrivateFieldGet(this, _SessionToolRunner_answered, "f").add(ev.custom_tool_use_id);
	...
	if (verdict === void 0) {
		if (permission === void 0 || permission === "allow") await __classPrivateFieldGet(this, _SessionToolRunner_instances, "m", _SessionToolRunner_execute).call(this, ev, void 0);
	...
	try {
		...
		if (__classPrivateFieldGet(this, _SessionToolRunner_inFlightCount, "f") === 0) __classPrivateFieldGet(this, _SessionToolRunner_onIdle, "f")?.call(this);
	}
	...
	for (let i = 0; i < SEND_RETRIES; i++) {
		...
		try {
			...
			if (i < SEND_RETRIES - 1) await sleep((i + 1) * 1e3, ctrl.signal);
		}
	}
	...
	return false;
}, _SessionToolRunner_drain = async function _SessionToolRunner_drain() {
	if (__classPrivateFieldGet(this, _SessionToolRunner_inFlightCount, "f") === 0) return;
	...
	if (__classPrivateFieldGet(this, _SessionToolRunner_inFlightCount, "f") > 0) __classPrivateFieldGet(this, _SessionToolRunner_logger, "f").warn("drain timeout exceeded");
};
/**
...
function buildResultEvent(ev, isError, content) {
	if (ev.type === "agent.custom_tool_use") return {
...
function toSessionContent(content) {
	...
	const out = content.map((b) => {
		if (b.type === "text") return {
			type: "text",
			text: b.text || "(no output)"
		};
		if (b.type === "image" || b.type === "document") return b;
		if (b.type === "search_result") return {
			type: "search_result",
			source: b.source,
			title: b.title,
...
//#endregion
//#region node_modules/@anthropic-ai/sdk/lib/environments/worker.mjs
...
var EnvironmentWorker = class {
	...
	*/
	async run(signal) {
		...
		if (environmentId === void 0 || environmentKey === void 0) throw new AnthropicError("EnvironmentWorker.run: environmentId and environmentKey are required to poll for work");
		...
		for await (const work of poller) await __classPrivateFieldGet(this, _EnvironmentWorker_instances, "m", _EnvironmentWorker_handleItem).call(this, work, environmentKey, poller.signal);
	}
	/**
	...
	*/
	async handleItem(opts) {
		...
		if (!workId) throw new AnthropicError("handleItem: workId is required — pass it or set ANTHROPIC_WORK_ID");
		if (!environmentId) throw new AnthropicError("handleItem: environmentId is required — pass it or set ANTHROPIC_ENVIRONMENT_ID");
		if (!sessionId) throw new AnthropicError("handleItem: sessionId is required — pass it or set ANTHROPIC_SESSION_ID");
		if (!environmentKey) throw new AnthropicError("handleItem: environmentKey is required — pass it, construct the worker with it, or set ANTHROPIC_ENVIRONMENT_KEY");
		const work = {
			id: workId,
...
_EnvironmentWorker_signal = /* @__PURE__ */ new WeakMap(), _EnvironmentWorker_instances = /* @__PURE__ */ new WeakSet(), _EnvironmentWorker_handleItem = async function _EnvironmentWorker_handleItem(work, environmentKey, externalSignal) {
	const log = loggerFor(this.client);
	...
	const agentToolset = await Promise.resolve().then(() => node_exports);
	let cleanupSkills = async () => {};
	try {
		cleanupSkills = await agentToolset.setupSkills(ctx);
	} catch (e) {
		log.warn("skill setup failed", {
	...
	const heartbeatPromise = heartbeatLoop(sessionClient, work, ctrl, log, this.requestOptions).catch((e) => {
		if (!ctrl.signal.aborted) log.error("heartbeat loop failed", {
			work_id: work.id,
			error: String(e)
		});
		ctrl.abort();
...
async function forceStop(client, work, log, requestOptions) {
	try {
		...
		if (!isStatus(e, 409)) log.error("force-stop on exit failed", {
			work_id: work.id,
			error: String(e)
		});
	}
...
async function heartbeatLoop(client, work, ctrl, logger, requestOptions) {
	...
	const beat = async () => {
		try {
			...
			if (resp.ttl_seconds > 0) intervalMs = Math.max(1e3, Math.min(resp.ttl_seconds * 1e3 / 2, HEARTBEAT_DEFAULT_MS));
			if (resp.state === "stopping" || resp.state === "stopped") {
				logger.info("heartbeat signals shutdown", {
					work_id: work.id,
					state: resp.state
				});
				ctrl.abort();
			...
			ctrl.signal.throwIfAborted();
			if (isFatal4xx(e)) {
				logger.error("permanent heartbeat failure", {
					work_id: work.id,
					error: String(e)
				});
				ctrl.abort();
			...
			logger.warn("transient heartbeat failure", {
				work_id: work.id,
				error: String(e)
			});
		}
...
//#endregion
//#region node_modules/@anthropic-ai/sdk/resources/beta/environments/work.mjs
var Work = class extends APIResource {
	...
	*/
	retrieve(workID, params, options) {
		...
		return this._client.get(path$2`/v1/environments/${environment_id}/work/${workID}?beta=true`, {
			...options,
			headers: buildHeaders([{ "anthropic-beta": [...betas ?? [], "managed-agents-2026-04-01"].toString() }, options?.headers])
		});
	}
	...
	*/
	update(workID, params, options) {
		...
		return this._client.post(path$2`/v1/environments/${environment_id}/work/${workID}?beta=true`, {
			body,
			...options,
			headers: buildHeaders([{ "anthropic-beta": [...betas ?? [], "managed-agents-2026-04-01"].toString() }, options?.headers])
		});
	...
	*/
	list(environmentID, params = {}, options) {
		const { betas, ...query } = params ?? {};
		return this._client.getAPIList(path$2`/v1/environments/${environmentID}/work?beta=true`, PageCursor, {
			query,
			...options,
			headers: buildHeaders([{ "anthropic-beta": [...betas ?? [], "managed-agents-2026-04-01"].toString() }, options?.headers])
		});
	...
	*/
	ack(workID, params, options) {
		const { environment_id, betas } = params;
		return this._client.post(path$2`/v1/environments/${environment_id}/work/${workID}/ack?beta=true`, {
			...options,
			headers: buildHeaders([{ "anthropic-beta": [...betas ?? [], "managed-agents-2026-04-01"].toString() }, options?.headers])
		});
	}
	...
	*/
	heartbeat(workID, params, options) {
		...
		return this._client.post(path$2`/v1/environments/${environment_id}/work/${workID}/heartbeat?beta=true`, {
			query: {
				desired_ttl_seconds,
	...
	*/
	poll(environmentID, params = {}, options) {
		...
		return this._client.get(path$2`/v1/environments/${environmentID}/work/poll?beta=true`, {
			query,
			...options,
			headers: buildHeaders([{
	...
	*/
	stats(environmentID, params = {}, options) {
		const { betas } = params ?? {};
		return this._client.get(path$2`/v1/environments/${environmentID}/work/stats?beta=true`, {
			...options,
			headers: buildHeaders([{ "anthropic-beta": [...betas ?? [], "managed-agents-2026-04-01"].toString() }, options?.headers])
		});
	}
	...
	*/
	stop(workID, params, options) {
		const { environment_id, betas, ...body } = params;
		return this._client.post(path$2`/v1/environments/${environment_id}/work/${workID}/stop?beta=true`, {
			body,
			...options,
			headers: buildHeaders([{ "anthropic-beta": [...betas ?? [], "managed-agents-2026-04-01"].toString() }, options?.headers])
		});
...
//#endregion
//#region node_modules/@anthropic-ai/sdk/resources/beta/environments/environments.mjs
var Environments = class extends APIResource {
	...
	*/
	retrieve(environmentID, params = {}, options) {
		const { betas } = params ?? {};
		return this._client.get(path$2`/v1/environments/${environmentID}?beta=true`, {
			...options,
			headers: buildHeaders([{ "anthropic-beta": [...betas ?? [], "managed-agents-2026-04-01"].toString() }, options?.headers])
		});
	}
	...
	*/
	update(environmentID, params, options) {
		const { betas, ...body } = params;
		return this._client.post(path$2`/v1/environments/${environmentID}?beta=true`, {
			body,
			...options,
			headers: buildHeaders([{ "anthropic-beta": [...betas ?? [], "managed-agents-2026-04-01"].toString() }, options?.headers])
		});
	...
	*/
	delete(environmentID, params = {}, options) {
		const { betas } = params ?? {};
		return this._client.delete(path$2`/v1/environments/${environmentID}?beta=true`, {
			...options,
			headers: buildHeaders([{ "anthropic-beta": [...betas ?? [], "managed-agents-2026-04-01"].toString() }, options?.headers])
		});
	}
	...
	*/
	archive(environmentID, params = {}, options) {
		const { betas } = params ?? {};
		return this._client.post(path$2`/v1/environments/${environmentID}/archive?beta=true`, {
			...options,
			headers: buildHeaders([{ "anthropic-beta": [...betas ?? [], "managed-agents-2026-04-01"].toString() }, options?.headers])
		});
	}
...
//#endregion
//#region node_modules/@anthropic-ai/sdk/resources/beta/memory-stores/memories.mjs
var Memories = class extends APIResource {
	...
	*/
	create(memoryStoreID, params, options) {
		...
		return this._client.post(path$2`/v1/memory_stores/${memoryStoreID}/memories?beta=true`, {
			query: { view },
			body,
			...options,
	...
	*/
	retrieve(memoryID, params, options) {
		...
		return this._client.get(path$2`/v1/memory_stores/${memory_store_id}/memories/${memoryID}?beta=true`, {
			query,
			...options,
			headers: buildHeaders([{ "anthropic-beta": [...betas ?? [], "agent-memory-2026-07-22"].toString() }, options?.headers])
		});
	...
	*/
	update(memoryID, params, options) {
		...
		return this._client.post(path$2`/v1/memory_stores/${memory_store_id}/memories/${memoryID}?beta=true`, {
			query: { view },
			body,
			...options,
			headers: buildHeaders([{ "anthropic-beta": [...betas ?? [], "agent-memory-2026-07-22"].toString() }, options?.headers])
	...
	*/
	list(memoryStoreID, params = {}, options) {
		const { betas, ...query } = params ?? {};
		return this._client.getAPIList(path$2`/v1/memory_stores/${memoryStoreID}/memories?beta=true`, PageCursor, {
			query,
			...options,
			headers: buildHeaders([{ "anthropic-beta": [...betas ?? [], "agent-memory-2026-07-22"].toString() }, options?.headers])
		});
	...
	*/
	delete(memoryID, params, options) {
		...
		return this._client.delete(path$2`/v1/memory_stores/${memory_store_id}/memories/${memoryID}?beta=true`, {
...
//#endregion
//#region node_modules/@anthropic-ai/sdk/resources/beta/memory-stores/memory-versions.mjs
var MemoryVersions = class extends APIResource {
	...
	*/
	retrieve(memoryVersionID, params, options) {
		const { memory_store_id, betas, ...query } = params;
		return this._client.get(path$2`/v1/memory_stores/${memory_store_id}/memory_versions/${memoryVersionID}?beta=true`, {
			query,
			...options,
			headers: buildHeaders([{ "anthropic-beta": [...betas ?? [], "agent-memory-2026-07-22"].toString() }, options?.headers])
		});
	...
	*/
	list(memoryStoreID, params = {}, options) {
		const { betas, ...query } = params ?? {};
		return this._client.getAPIList(path$2`/v1/memory_stores/${memoryStoreID}/memory_versions?beta=true`, PageCursor, {
			query,
			...options,
			headers: buildHeaders([{ "anthropic-beta": [...betas ?? [], "agent-memory-2026-07-22"].toString() }, options?.headers])
		});
	...
	*/
	redact(memoryVersionID, params, options) {
		...
		return this._client.post(path$2`/v1/memory_stores/${memory_store_id}/memory_versions/${memoryVersionID}/redact?beta=true`, {
			...options,
			headers: buildHeaders([{ "anthropic-beta": [...betas ?? [], "agent-memory-2026-07-22"].toString() }, options?.headers])
		});
	}
...
//#endregion
//#region node_modules/@anthropic-ai/sdk/resources/beta/memory-stores/memory-stores.mjs
var MemoryStores = class extends APIResource {
	...
	*/
	retrieve(memoryStoreID, params = {}, options) {
		const { betas } = params ?? {};
		return this._client.get(path$2`/v1/memory_stores/${memoryStoreID}?beta=true`, {
			...options,
			headers: buildHeaders([{ "anthropic-beta": [...betas ?? [], "agent-memory-2026-07-22"].toString() }, options?.headers])
		});
	}
	...
	*/
	update(memoryStoreID, params, options) {
		const { betas, ...body } = params;
		return this._client.post(path$2`/v1/memory_stores/${memoryStoreID}?beta=true`, {
			body,
			...options,
			headers: buildHeaders([{ "anthropic-beta": [...betas ?? [], "agent-memory-2026-07-22"].toString() }, options?.headers])
		});
	...
	*/
	delete(memoryStoreID, params = {}, options) {
		const { betas } = params ?? {};
		return this._client.delete(path$2`/v1/memory_stores/${memoryStoreID}?beta=true`, {
			...options,
			headers: buildHeaders([{ "anthropic-beta": [...betas ?? [], "agent-memory-2026-07-22"].toString() }, options?.headers])
		});
	}
	...
	*/
	archive(memoryStoreID, params = {}, options) {
		const { betas } = params ?? {};
		return this._client.post(path$2`/v1/memory_stores/${memoryStoreID}/archive?beta=true`, {
			...options,
			headers: buildHeaders([{ "anthropic-beta": [...betas ?? [], "agent-memory-2026-07-22"].toString() }, options?.headers])
		});
	}
...
var Batches$1 = class extends APIResource {
	...
	*/
	retrieve(messageBatchID, params = {}, options) {
		const { betas } = params ?? {};
		return this._client.get(path$2`/v1/messages/batches/${messageBatchID}?beta=true`, {
			...options,
	...
	*/
	delete(messageBatchID, params = {}, options) {
		const { betas } = params ?? {};
		return this._client.delete(path$2`/v1/messages/batches/${messageBatchID}?beta=true`, {
			...options,
			headers: buildHeaders([{ "anthropic-beta": [...betas ?? [], "message-batches-2024-09-24"].toString() }, options?.headers])
		});
	}
	...
	*/
	cancel(messageBatchID, params = {}, options) {
		const { betas } = params ?? {};
		return this._client.post(path$2`/v1/messages/batches/${messageBatchID}/cancel?beta=true`, {
			...options,
			headers: buildHeaders([{ "anthropic-beta": [...betas ?? [], "message-batches-2024-09-24"].toString() }, options?.headers])
		});
	}
...
var tokenize = (input) => {
	...
	while (current < input.length) {
		...
		if (char && NUMBERS.test(char) || char === "-" || char === ".") {
			...
			while (char && (NUMBERS.test(char) || char === "." || char === "e" || char === "E" || (char === "-" || char === "+") && (value[value.length - 1] === "e" || value[value.length - 1] === "E"))) {
				value += char;
				char = input[++current];
			}
			tokens.push({
...
//#endregion
//#region node_modules/@anthropic-ai/sdk/internal/message-stream-utils.mjs
...
function withLazyInput(prev, jsonBuf) {
	const next = {};
	for (const key of Object.keys(prev)) if (key !== "input") next[key] = prev[key];
	...
	[(_BetaMessageStream_currentMessageSnapshot = /* @__PURE__ */ new WeakMap(), _BetaMessageStream_params = /* @__PURE__ */ new WeakMap(), _BetaMessageStream_connectedPromise = /* @__PURE__ */ new WeakMap(), _BetaMessageStream_resolveConnectedPromise = /* @__PURE__ */ new WeakMap(), _BetaMessageStream_rejectConnectedPromise = /* @__PURE__ */ new WeakMap(), _BetaMessageStream_endPromise = /* @__PURE__ */ new WeakMap(), _BetaMessageStream_resolveEndPromise = /* @__PURE__ */ new WeakMap(), _BetaMessageStream_rejectEndPromise = /* @__PURE__ */ new WeakMap(), _BetaMessageStream_listeners = /* @__PURE__ */ new WeakMap(), _BetaMessageStream_ended = /* @__PURE__ */ new WeakMap(), _BetaMessageStream_errored = /* @__PURE__ */ new WeakMap(), _BetaMessageStream_aborted = /* @__PURE__ */ new WeakMap(), _BetaMessageStream_catchingPromiseCreated = /* @__PURE__ */ new WeakMap(), _BetaMessageStream_response = /* @__PURE__ */ new WeakMap(), _BetaMessageStream_request_id = /* @__PURE__ */ new WeakMap(), _BetaMessageStream_logger = /* @__PURE__ */ new WeakMap(), _BetaMessageStream_handleError = /* @__PURE__ */ new WeakMap(), _BetaMessageStream_instances = /* @__PURE__ */ new WeakSet(), _BetaMessageStream_getFinalMessage = function _BetaMessageStream_getFinalMessage() {
		...
		switch (event.type) {
			...
			case "message_delta":
				...
				if (event.delta.stop_details != null) snapshot.stop_details = event.delta.stop_details;
				...
				if (event.usage.fallback_credit != null) snapshot.usage.fallback_credit = event.usage.fallback_credit;
				return snapshot;
			case "content_block_start":
				snapshot.content.push(event.content_block);
				if (event.content_block.type === "fallback") snapshot.model = event.content_block.to.model;
				return snapshot;
			case "content_block_delta": {
...
//#endregion
//#region node_modules/@anthropic-ai/sdk/internal/utils/promise.mjs
/**
...
async function generateToolResponse(params, lastMessage = params.messages.at(-1), requestOptions) {
	...
	return {
		...
		content: await Promise.all(toolUseBlocks.map(async (toolUse) => {
			...
			if (!tool || !("run" in tool) || !available.has(toolUse.name)) return toolNotFoundResult(toolUse);
			try {
...
function availableToolNames(params) {
	...
	for (const tool of params.tools) if ("run" in tool) available.add(tool.name);
	for (const message of params.messages) {
		if (message.role !== "system" || typeof message.content === "string") continue;
		for (const block of message.content) applyToolChange(block, available);
	}
	return available;
}
function applyToolChange(block, available) {
	switch (block.type) {
		...
		case "mid_conv_system":
			for (const inner of block.content) if (inner.type === "tool_removal" || inner.type === "tool_addition") applyToolReference(inner, available);
			break;
		default: break;
	}
}
function applyToolReference(block, available) {
	...
	if (block.type === "tool_removal") available.delete(name);
	else available.add(name);
}
function referencedToolName(ref) {
	switch (ref.type) {
		case "tool_reference": return ref.name;
		default: return;
	}
}
//#endregion
...
var Messages$1 = class extends APIResource {
	...
	*/
	countTokens(params, options) {
		const { betas, user_profile_id, ...body } = transformOutputFormat(params);
...
//#endregion
//#region node_modules/@anthropic-ai/sdk/resources/beta/sessions/events.mjs
var Events$1 = class extends APIResource {
	...
	*/
	list(sessionID, params = {}, options) {
		const { betas, ...query } = params ?? {};
		return this._client.getAPIList(path$2`/v1/sessions/${sessionID}/events?beta=true`, PageCursor, {
			query,
			...options,
			headers: buildHeaders([{ "anthropic-beta": [...betas ?? [], "managed-agents-2026-04-01"].toString() }, options?.headers])
		});
	...
	*/
	send(sessionID, params, options) {
		const { betas, ...body } = params;
		return this._client.post(path$2`/v1/sessions/${sessionID}/events?beta=true`, {
			body,
			...options,
			headers: buildHeaders([{ "anthropic-beta": [...betas ?? [], "managed-agents-2026-04-01"].toString() }, options?.headers])
		});
	...
	*/
	stream(sessionID, params = {}, options) {
		const { betas, ...query } = params ?? {};
		return this._client.get(path$2`/v1/sessions/${sessionID}/events/stream?beta=true`, {
			query,
			...options,
			headers: buildHeaders([{ "anthropic-beta": [...betas ?? [], "managed-agents-2026-04-01"].toString() }, options?.headers]),
			stream: true
	...
	*/
	toolRunner(sessionID, opts) {
		return new SessionToolRunner(sessionID, {
			...opts,
			client: this._client
		});
	}
...
//#endregion
//#region node_modules/@anthropic-ai/sdk/resources/beta/sessions/resources.mjs
var Resources = class extends APIResource {
	...
	*/
	retrieve(resourceID, params, options) {
		...
		return this._client.get(path$2`/v1/sessions/${session_id}/resources/${resourceID}?beta=true`, {
			...options,
			headers: buildHeaders([{ "anthropic-beta": [...betas ?? [], "managed-agents-2026-04-01"].toString() }, options?.headers])
		});
	}
	...
	*/
	update(resourceID, params, options) {
		...
		return this._client.post(path$2`/v1/sessions/${session_id}/resources/${resourceID}?beta=true`, {
			body,
			...options,
			headers: buildHeaders([{ "anthropic-beta": [...betas ?? [], "managed-agents-2026-04-01"].toString() }, options?.headers])
		});
	...
	*/
	list(sessionID, params = {}, options) {
		const { betas, ...query } = params ?? {};
		return this._client.getAPIList(path$2`/v1/sessions/${sessionID}/resources?beta=true`, PageCursor, {
			query,
			...options,
			headers: buildHeaders([{ "anthropic-beta": [...betas ?? [], "managed-agents-2026-04-01"].toString() }, options?.headers])
		});
	...
	*/
	delete(resourceID, params, options) {
		const { session_id, betas } = params;
		return this._client.delete(path$2`/v1/sessions/${session_id}/resources/${resourceID}?beta=true`, {
			...options,
			headers: buildHeaders([{ "anthropic-beta": [...betas ?? [], "managed-agents-2026-04-01"].toString() }, options?.headers])
		});
	}
	...
	*/
	add(sessionID, params, options) {
		const { betas, ...body } = params;
		return this._client.post(path$2`/v1/sessions/${sessionID}/resources?beta=true`, {
			body,
			...options,
			headers: buildHeaders([{ "anthropic-beta": [...betas ?? [], "managed-agents-2026-04-01"].toString() }, options?.headers])
		});
...
//#endregion
//#region node_modules/@anthropic-ai/sdk/resources/beta/sessions/threads/events.mjs
var Events = class extends APIResource {
	...
	*/
	list(threadID, params, options) {
		...
		return this._client.getAPIList(path$2`/v1/sessions/${session_id}/threads/${threadID}/events?beta=true`, PageCursor, {
			query,
			...options,
			headers: buildHeaders([{ "anthropic-beta": [...betas ?? [], "managed-agents-2026-04-01"].toString() }, options?.headers])
		});
	...
	*/
	stream(threadID, params, options) {
		const { session_id, betas, ...query } = params;
		return this._client.get(path$2`/v1/sessions/${session_id}/threads/${threadID}/stream?beta=true`, {
			query,
			...options,
			headers: buildHeaders([{ "anthropic-beta": [...betas ?? [], "managed-agents-2026-04-01"].toString() }, options?.headers]),
			stream: true
...
//#endregion
//#region node_modules/@anthropic-ai/sdk/resources/beta/sessions/threads/threads.mjs
var Threads = class extends APIResource {
	...
	*/
	retrieve(threadID, params, options) {
		const { session_id, betas } = params;
		return this._client.get(path$2`/v1/sessions/${session_id}/threads/${threadID}?beta=true`, {
			...options,
			headers: buildHeaders([{ "anthropic-beta": [...betas ?? [], "managed-agents-2026-04-01"].toString() }, options?.headers])
		});
	}
	...
	*/
	list(sessionID, params = {}, options) {
		const { betas, ...query } = params ?? {};
		return this._client.getAPIList(path$2`/v1/sessions/${sessionID}/threads?beta=true`, PageCursor, {
			query,
			...options,
			headers: buildHeaders([{ "anthropic-beta": [...betas ?? [], "managed-agents-2026-04-01"].toString() }, options?.headers])
		});
	...
	*/
	archive(threadID, params, options) {
		const { session_id, betas } = params;
		return this._client.post(path$2`/v1/sessions/${session_id}/threads/${threadID}/archive?beta=true`, {
			...options,
			headers: buildHeaders([{ "anthropic-beta": [...betas ?? [], "managed-agents-2026-04-01"].toString() }, options?.headers])
		});
	}
...
//#endregion
//#region node_modules/@anthropic-ai/sdk/resources/beta/sessions/sessions.mjs
var Sessions = class extends APIResource {
	...
	*/
	retrieve(sessionID, params = {}, options) {
		const { betas } = params ?? {};
		return this._client.get(path$2`/v1/sessions/${sessionID}?beta=true`, {
			...options,
			headers: buildHeaders([{ "anthropic-beta": [...betas ?? [], "managed-agents-2026-04-01"].toString() }, options?.headers])
		});
	}
	...
	*/
	update(sessionID, params, options) {
		const { betas, ...body } = params;
		return this._client.post(path$2`/v1/sessions/${sessionID}?beta=true`, {
			body,
			...options,
			headers: buildHeaders([{ "anthropic-beta": [...betas ?? [], "managed-agents-2026-04-01"].toString() }, options?.headers])
		});
	...
	*/
	delete(sessionID, params = {}, options) {
		const { betas } = params ?? {};
		return this._client.delete(path$2`/v1/sessions/${sessionID}?beta=true`, {
			...options,
			headers: buildHeaders([{ "anthropic-beta": [...betas ?? [], "managed-agents-2026-04-01"].toString() }, options?.headers])
		});
	}
	...
	*/
	archive(sessionID, params = {}, options) {
		const { betas } = params ?? {};
		return this._client.post(path$2`/v1/sessions/${sessionID}/archive?beta=true`, {
			...options,
			headers: buildHeaders([{ "anthropic-beta": [...betas ?? [], "managed-agents-2026-04-01"].toString() }, options?.headers])
		});
	}
...
var Versions = class extends APIResource {
	...
	*/
	create(skillID, params, options) {
		const { betas, ...body } = params;
		return this._client.post(path$2`/v1/skills/${skillID}/versions?beta=true`, multipartFormRequestOptions({
			body,
			...options,
	...
	*/
	retrieve(version, params, options) {
		...
		return this._client.get(path$2`/v1/skills/${skill_id}/versions/${version}?beta=true`, {
			...options,
			headers: buildHeaders([{ "anthropic-beta": [...betas ?? [], "skills-2025-10-02"].toString() }, options?.headers])
		});
	}
	...
	*/
	list(skillID, params = {}, options) {
		const { betas, ...query } = params ?? {};
		return this._client.getAPIList(path$2`/v1/skills/${skillID}/versions?beta=true`, PageCursor, {
			query,
			...options,
			headers: buildHeaders([{ "anthropic-beta": [...betas ?? [], "skills-2025-10-02"].toString() }, options?.headers])
		});
	...
	*/
	delete(version, params, options) {
		const { skill_id, betas } = params;
		return this._client.delete(path$2`/v1/skills/${skill_id}/versions/${version}?beta=true`, {
			...options,
			headers: buildHeaders([{ "anthropic-beta": [...betas ?? [], "skills-2025-10-02"].toString() }, options?.headers])
		});
	}
	...
	*/
	download(version, params, options) {
		const { skill_id, betas } = params;
		return this._client.get(path$2`/v1/skills/${skill_id}/versions/${version}/content?beta=true`, {
			...options,
			headers: buildHeaders([{
...
var Skills = class extends APIResource {
	...
	*/
	retrieve(skillID, params = {}, options) {
		const { betas } = params ?? {};
		return this._client.get(path$2`/v1/skills/${skillID}?beta=true`, {
			...options,
			headers: buildHeaders([{ "anthropic-beta": [...betas ?? [], "skills-2025-10-02"].toString() }, options?.headers])
		});
	}
	...
	*/
	delete(skillID, params = {}, options) {
		const { betas } = params ?? {};
		return this._client.delete(path$2`/v1/skills/${skillID}?beta=true`, {
			...options,
			headers: buildHeaders([{ "anthropic-beta": [...betas ?? [], "skills-2025-10-02"].toString() }, options?.headers])
		});
	}
...
//#endregion
//#region node_modules/@anthropic-ai/sdk/resources/beta/tunnels/certificates.mjs
var Certificates = class extends APIResource {
	...
	*/
	create(tunnelID, params, options) {
		const { betas, ...body } = params;
		return this._client.post(path$2`/v1/tunnels/${tunnelID}/certificates?beta=true`, {
			body,
			...options,
	...
	*/
	retrieve(certificateID, params, options) {
		...
		return this._client.get(path$2`/v1/tunnels/${tunnel_id}/certificates/${certificateID}?beta=true`, {
			...options,
			headers: buildHeaders([{ "anthropic-beta": [...betas ?? [], "mcp-tunnels-2026-06-22"].toString() }, options?.headers])
		});
	}
	...
	*/
	list(tunnelID, params = {}, options) {
		const { betas, ...query } = params ?? {};
		return this._client.getAPIList(path$2`/v1/tunnels/${tunnelID}/certificates?beta=true`, PageCursor, {
			query,
			...options,
			headers: buildHeaders([{ "anthropic-beta": [...betas ?? [], "mcp-tunnels-2026-06-22"].toString() }, options?.headers])
		});
	...
	*/
	archive(certificateID, params, options) {
		const { tunnel_id, betas } = params;
		return this._client.post(path$2`/v1/tunnels/${tunnel_id}/certificates/${certificateID}/archive?beta=true`, {
			...options,
			headers: buildHeaders([{ "anthropic-beta": [...betas ?? [], "mcp-tunnels-2026-06-22"].toString() }, options?.headers])
		});
	}
...
//#endregion
//#region node_modules/@anthropic-ai/sdk/resources/beta/tunnels/tunnels.mjs
var Tunnels = class extends APIResource {
	...
	*/
	retrieve(tunnelID, params = {}, options) {
		const { betas } = params ?? {};
		return this._client.get(path$2`/v1/tunnels/${tunnelID}?beta=true`, {
			...options,
			headers: buildHeaders([{ "anthropic-beta": [...betas ?? [], "mcp-tunnels-2026-06-22"].toString() }, options?.headers])
		});
	}
	...
	*/
	archive(tunnelID, params = {}, options) {
		const { betas } = params ?? {};
		return this._client.post(path$2`/v1/tunnels/${tunnelID}/archive?beta=true`, {
			...options,
			headers: buildHeaders([{ "anthropic-beta": [...betas ?? [], "mcp-tunnels-2026-06-22"].toString() }, options?.headers])
		});
	}
	...
	*/
	revealToken(tunnelID, params = {}, options) {
		const { betas } = params ?? {};
		return this._client.post(path$2`/v1/tunnels/${tunnelID}/reveal_token?beta=true`, {
			...options,
			headers: buildHeaders([{ "anthropic-beta": [...betas ?? [], "mcp-tunnels-2026-06-22"].toString() }, options?.headers])
		});
	}
	...
	*/
	rotateToken(tunnelID, params, options) {
		const { betas, ...body } = params;
		return this._client.post(path$2`/v1/tunnels/${tunnelID}/rotate_token?beta=true`, {
			body,
			...options,
			headers: buildHeaders([{ "anthropic-beta": [...betas ?? [], "mcp-tunnels-2026-06-22"].toString() }, options?.headers])
		});
...
//#endregion
//#region node_modules/@anthropic-ai/sdk/resources/beta/vaults/credentials.mjs
var Credentials = class extends APIResource {
	...
	*/
	create(vaultID, params, options) {
		const { betas, ...body } = params;
		return this._client.post(path$2`/v1/vaults/${vaultID}/credentials?beta=true`, {
			body,
			...options,
			headers: buildHeaders([{ "anthropic-beta": [...betas ?? [], "managed-agents-2026-04-01"].toString() }, options?.headers])
		});
	...
	*/
	retrieve(credentialID, params, options) {
		...
		return this._client.get(path$2`/v1/vaults/${vault_id}/credentials/${credentialID}?beta=true`, {
			...options,
			headers: buildHeaders([{ "anthropic-beta": [...betas ?? [], "managed-agents-2026-04-01"].toString() }, options?.headers])
		});
	}
	...
	*/
	update(credentialID, params, options) {
		...
		return this._client.post(path$2`/v1/vaults/${vault_id}/credentials/${credentialID}?beta=true`, {
			body,
			...options,
			headers: buildHeaders([{ "anthropic-beta": [...betas ?? [], "managed-agents-2026-04-01"].toString() }, options?.headers])
		});
	...
	*/
	list(vaultID, params = {}, options) {
		const { betas, ...query } = params ?? {};
		return this._client.getAPIList(path$2`/v1/vaults/${vaultID}/credentials?beta=true`, PageCursor, {
			query,
			...options,
			headers: buildHeaders([{ "anthropic-beta": [...betas ?? [], "managed-agents-2026-04-01"].toString() }, options?.headers])
		});
	...
	*/
	delete(credentialID, params, options) {
		const { vault_id, betas } = params;
		return this._client.delete(path$2`/v1/vaults/${vault_id}/credentials/${credentialID}?beta=true`, {
			...options,
			headers: buildHeaders([{ "anthropic-beta": [...betas ?? [], "managed-agents-2026-04-01"].toString() }, options?.headers])
		});
	}
	...
	*/
	archive(credentialID, params, options) {
		const { vault_id, betas } = params;
		return this._client.post(path$2`/v1/vaults/${vault_id}/credentials/${credentialID}/archive?beta=true`, {
			...options,
			headers: buildHeaders([{ "anthropic-beta": [...betas ?? [], "managed-agents-2026-04-01"].toString() }, options?.headers])
		});
	}
	...
	*/
	mcpOAuthValidate(credentialID, params, options) {
		const { vault_id, betas } = params;
		return this._client.post(path$2`/v1/vaults/${vault_id}/credentials/${credentialID}/mcp_oauth_validate?beta=true`, {
			...options,
			headers: buildHeaders([{ "anthropic-beta": [...betas ?? [], "managed-agents-2026-04-01"].toString() }, options?.headers])
		});
	}
...
//#endregion
//#region node_modules/@anthropic-ai/sdk/resources/beta/vaults/vaults.mjs
var Vaults = class extends APIResource {
	...
	*/
	retrieve(vaultID, params = {}, options) {
		const { betas } = params ?? {};
		return this._client.get(path$2`/v1/vaults/${vaultID}?beta=true`, {
			...options,
			headers: buildHeaders([{ "anthropic-beta": [...betas ?? [], "managed-agents-2026-04-01"].toString() }, options?.headers])
		});
	}
	...
	*/
	update(vaultID, params, options) {
		const { betas, ...body } = params;
		return this._client.post(path$2`/v1/vaults/${vaultID}?beta=true`, {
			body,
			...options,
			headers: buildHeaders([{ "anthropic-beta": [...betas ?? [], "managed-agents-2026-04-01"].toString() }, options?.headers])
		});
	...
	*/
	delete(vaultID, params = {}, options) {
		const { betas } = params ?? {};
		return this._client.delete(path$2`/v1/vaults/${vaultID}?beta=true`, {
			...options,
			headers: buildHeaders([{ "anthropic-beta": [...betas ?? [], "managed-agents-2026-04-01"].toString() }, options?.headers])
		});
	}
	...
	*/
	archive(vaultID, params = {}, options) {
		const { betas } = params ?? {};
		return this._client.post(path$2`/v1/vaults/${vaultID}/archive?beta=true`, {
			...options,
			headers: buildHeaders([{ "anthropic-beta": [...betas ?? [], "managed-agents-2026-04-01"].toString() }, options?.headers])
		});
	}
	...
	*/
	...
	[(_MessageStream_currentMessageSnapshot = /* @__PURE__ */ new WeakMap(), _MessageStream_params = /* @__PURE__ */ new WeakMap(), _MessageStream_connectedPromise = /* @__PURE__ */ new WeakMap(), _MessageStream_resolveConnectedPromise = /* @__PURE__ */ new WeakMap(), _MessageStream_rejectConnectedPromise = /* @__PURE__ */ new WeakMap(), _MessageStream_endPromise = /* @__PURE__ */ new WeakMap(), _MessageStream_resolveEndPromise = /* @__PURE__ */ new WeakMap(), _MessageStream_rejectEndPromise = /* @__PURE__ */ new WeakMap(), _MessageStream_listeners = /* @__PURE__ */ new WeakMap(), _MessageStream_ended = /* @__PURE__ */ new WeakMap(), _MessageStream_errored = /* @__PURE__ */ new WeakMap(), _MessageStream_aborted = /* @__PURE__ */ new WeakMap(), _MessageStream_catchingPromiseCreated = /* @__PURE__ */ new WeakMap(), _MessageStream_response = /* @__PURE__ */ new WeakMap(), _MessageStream_request_id = /* @__PURE__ */ new WeakMap(), _MessageStream_logger = /* @__PURE__ */ new WeakMap(), _MessageStream_handleError = /* @__PURE__ */ new WeakMap(), _MessageStream_instances = /* @__PURE__ */ new WeakSet(), _MessageStream_getFinalMessage = function _MessageStream_getFinalMessage() {
			...
			case "content_block_delta": {
				...
				switch (event.delta.type) {
					...
					case "citations_delta":
						...
						break;
					case "input_json_delta":
						if (tracksToolInput(content) && __classPrivateFieldGet(this, _MessageStream_listeners, "f").inputJson?.length) this._emit("inputJson", event.delta.partial_json, content.input);
						break;
					case "thinking_delta":
						if (content.type === "thinking") this._emit("thinking", event.delta.thinking, content.thinking);
						break;
		...
		switch (event.type) {
			...
			case "content_block_stop": {
				const snapshotContent = snapshot.content.at(event.index);
				if (snapshotContent && tracksToolInput(snapshotContent) && "__json_buf" in snapshotContent) Object.defineProperty(snapshotContent, "input", {
...
var Batches = class extends APIResource {
	...
	*/
	create(params, options) {
		...
		return this._client.post("/v1/messages/batches", {
			body,
			...options,
			headers: buildHeaders([{ ...user_profile_id != null ? { "anthropic-user-profile-id": user_profile_id } : void 0 }, options?.headers])
		});
	}
	/**
	* This endpoint is idempotent and can be used to poll for Message Batch
	...
	*/
	retrieve(messageBatchID, options) {
		return this._client.get(path$2`/v1/messages/batches/${messageBatchID}`, options);
	}
	/**
	* List all Message Batches within a Workspace. Most recently created batches are
	* returned first.
	...
	*/
	delete(messageBatchID, options) {
		return this._client.delete(path$2`/v1/messages/batches/${messageBatchID}`, options);
	}
	/**
	* Batches may be canceled any time before processing ends. Once cancellation is
	* initiated, the batch enters a `canceling` state, at which time the system may
	...
	*/
	cancel(messageBatchID, options) {
		return this._client.post(path$2`/v1/messages/batches/${messageBatchID}/cancel`, options);
	}
	/**
	* Streams the results of a Message Batch as a `.jsonl` file.
	*
...
var Models = class extends APIResource {
	...
	*/
	retrieve(modelID, params = {}, options) {
		const { betas } = params ?? {};
		return this._client.get(path$2`/v1/models/${modelID}`, {
			...options,
			headers: buildHeaders([{ ...betas?.toString() != null ? { "anthropic-beta": betas?.toString() } : void 0 }, options?.headers])
		});
	}
...
var BaseAnthropic = class {
	...
	*/
	constructor({ baseURL = readEnv("ANTHROPIC_BASE_URL"), apiKey, authToken, webhookKey = readEnv("ANTHROPIC_WEBHOOK_SIGNING_KEY") ?? null, ...opts } = {}) {
		...
		if (apiKey === void 0) apiKey = opts.profile != null ? null : readEnv("ANTHROPIC_API_KEY") ?? null;
		if (authToken === void 0) authToken = opts.profile != null ? null : readEnv("ANTHROPIC_AUTH_TOKEN") ?? null;
		if (opts.profile != null && (opts.credentials != null || opts.config != null)) throw new TypeError("Pass at most one of `profile`, `credentials`, or `config`.");
		const options = {
			apiKey,
			authToken,
			webhookKey,
		...
		if (inherited) {
			...
			if (!this._baseURLIsExplicit && inherited.baseURL) this.baseURL = inherited.baseURL;
		} else {
			this._authState = {
				provider: null,
				tokenCache: null,
			...
			if (this.apiKey == null && this.authToken == null) {
				...
				} else if (options.profile != null) this._authState.resolution = this._resolveDefaultCredentials(options.profile);
				else if (this._shouldResolveDefaultCredentials()) this._authState.resolution = this._resolveDefaultCredentials();
			}
		}
	}
	/**
	...
	*/
	_applyCredentialBaseURL(baseURL) {
		...
		if (!this._baseURLIsExplicit) this.baseURL = normalized;
	}
	/**
	...
	async _resolveDefaultCredentials(profile) {
		try {
			...
			if (result) {
				...
				this._authState.extraHeaders = result.extraHeaders;
				this._applyCredentialBaseURL(result.baseURL);
			} else if (profile != null) throw new AnthropicError(`Profile "${profile}" could not be resolved (no <config_dir>/configs/${profile}.json found).`);
		} catch (err) {
	...
	*/
	async prepareRequest(request, { url, options }) {
		if (this._authState.tokenCache && this.apiKey == null) {
			...
			for (const [k, v] of Object.entries(this._authState.extraHeaders)) if (!headers.has(k)) headers.set(k, v);
			if (!(headers.get("anthropic-beta")?.split(",").map((s) => s.trim()))?.includes("oauth-2025-04-20")) headers.append("anthropic-beta", OAUTH_API_BETA_HEADER);
	...
	*/
	...
	async makeRequest(optionsInput, retriesRemaining, retryOfRequestLogID) {
		...
		if (response instanceof globalThis.Error) {
			...
			if (hasMiddleware && !isFetchOriginError(response)) throw response;
	...
	async fetchWithTimeout(url, init, ms, controller, requestOptions, logCtx) {
		...
		const innerFetch = requestOptions === void 0 ? timedFetch : (async (innerUrl, innerInit = {}) => {
			...
			await this.prepareRequest(innerInit, {
				url: innerUrlStr,
				options: requestOptions
			});
			if (logCtx) loggerFor(this).debug(`[${logCtx.requestLogID}] sending request`, formatRequestDetails({
		...
		return await wrapFetchWithMiddleware(innerFetch, requestMiddleware?.length || backendMiddleware.length ? [
			...this.middleware,
			...requestMiddleware ?? [],
			...backendMiddleware
		] : this.middleware, requestOptions, this)(url, fetchOptions);
	}
...
new TextEncoder();
//#endregion
//#region node_modules/@anthropic-ai/sdk/lib/transform-json-schema.mjs
...
function transformJSONSchema(jsonSchema) {
	return _transformJSONSchema(deepClone(jsonSchema));
}
function _transformJSONSchema(jsonSchema) {
	...
	if (defs !== void 0) {
		...
		for (const [name, defSchema] of Object.entries(defs)) strictDefs[name] = _transformJSONSchema(defSchema);
	}
	...
	if (Array.isArray(anyOf)) strictSchema["anyOf"] = anyOf.map((variant) => _transformJSONSchema(variant));
	else if (Array.isArray(oneOf)) strictSchema["anyOf"] = oneOf.map((variant) => _transformJSONSchema(variant));
	else if (Array.isArray(allOf)) strictSchema["allOf"] = allOf.map((entry) => _transformJSONSchema(entry));
	else {
		if (type === void 0) throw new Error("JSON schema must have a type defined if anyOf/oneOf/allOf are not used");
		strictSchema["type"] = type;
	}
	const description = pop(jsonSchema, "description");
	if (description !== void 0) strictSchema["description"] = description;
	const title = pop(jsonSchema, "title");
	if (title !== void 0) strictSchema["title"] = title;
	if (type === "object") {
		...
		if (required !== void 0) strictSchema["required"] = required;
	} else if (type === "string") {
		const format = pop(jsonSchema, "format");
		if (format !== void 0 && SUPPORTED_STRING_FORMATS.has(format)) strictSchema["format"] = format;
		else if (format !== void 0) jsonSchema["format"] = format;
	} else if (type === "array") {
		const items = pop(jsonSchema, "items");
		if (items !== void 0) strictSchema["items"] = _transformJSONSchema(items);
		const minItems = pop(jsonSchema, "minItems");
		if (minItems !== void 0 && (minItems === 0 || minItems === 1)) strictSchema["minItems"] = minItems;
		else if (minItems !== void 0) jsonSchema["minItems"] = minItems;
	}
...
//#endregion
//#region node_modules/@anthropic-ai/sdk/helpers/beta/json-schema.mjs
/**
...
async function canonicalize(abs) {
	...
	for (;;) {
		...
		try {
			...
			if (isLink) {
				if (++hops > 40) throw new ToolError(`path ${JSON.stringify(abs)} has too many levels of symbolic links`);
...
async function confineToRoot(root, p, opts) {
	...
	if (real !== realRoot && !real.startsWith(realRoot + path$1.sep)) throw new ToolError(`path ${JSON.stringify(p)} escapes workdir`);
	return real;
}
/**
...
//#endregion
//#region node_modules/@anthropic-ai/sdk/tools/agent-toolset/skills.mjs
/**
...
async function setupSkills(ctx) {
	...
	return async () => {
		for (const dest of created) await fs$2.rm(dest, {
			recursive: true,
			force: true
		}).catch((e) => {
...
async function resolveSkillVersion(client, skillId, version) {
	...
	let newest;
	for await (const v of client.beta.skills.versions.list(skillId)) if (/^\d+$/.test(v.version) && (newest === void 0 || BigInt(v.version) > BigInt(newest))) newest = v.version;
	if (newest === void 0) throw new AnthropicError(`skill ${JSON.stringify(skillId)} has no concrete version to resolve ${JSON.stringify(version)} against`);
	return newest;
}
...
function assertNoSpecialMembers(verboseListing) {
	for (const line of verboseListing.split("\n")) {
		...
		if (type === "l" || type === "h" || type === "b" || type === "c" || type === "p" || type === "s") throw new AnthropicError("refusing to extract archive with symlink/hardlink/device member");
	}
}
/**
...
async function runArchiveTool(cmd, args) {
	try {
		...
		if (e != null && typeof e === "object" && e.code === "ENOENT") throw new AnthropicError(`skill extraction requires the \`${cmd}\` command, but it was not found on PATH`);
		throw e;
	}
}
/**
...
async function extractSkillArchive(resp, dest) {
	...
	try {
		...
		for (const entry of await fs$2.readdir(srcRoot)) await fs$2.rename(path$1.join(srcRoot, entry), path$1.join(dest, entry));
	} finally {
		await fs$2.rm(tmp, { force: true });
		await fs$2.rm(stage, {
			recursive: true,
			force: true
		});
...
async function readHead(file, n) {
	const handle = await fs$2.open(file, "r");
	try {
...
//#endregion
//#region node_modules/@anthropic-ai/sdk/tools/agent-toolset/node.mjs
...
var BashSession = class {
	...
	async exec(command, opts = {}) {
		...
		if (signal?.aborted) throw new AnthropicError("bash command aborted");
...
function betaReadTool(ctx) {
	return betaTool({
		...
		run: async ({ file_path, view_range }) => {
			...
			let data;
			try {
				const st = await fs$2.stat(abs);
				if (!st.isFile()) throw new ToolError(`read: ${file_path} is not a regular file`);
				const limit = resolveMaxBytes(ctx.maxFileBytes);
				if (limit !== null && st.size > limit) throw new ToolError(`read: ${file_path} is ${st.size} bytes, exceeds ${limit}-byte limit. Use bash (head/tail/sed) to read a slice.`);
				data = await fs$2.readFile(abs, "utf8");
			} catch (e) {
				if (e instanceof ToolError) throw e;
...
function betaWriteTool(ctx) {
	return betaTool({
		...
		run: async ({ file_path, content }) => {
			...
			const abs = await resolvePath(ctx, file_path);
			try {
				await fs$2.mkdir(path$1.dirname(abs), {
					recursive: true,
					mode: 493
				});
	...
	return betaTool({
		...
		run: async ({ file_path, old_string, new_string, replace_all }) => {
			...
			try {
				...
				const limit = resolveMaxBytes(ctx.maxFileBytes);
				if (limit !== null && st.size > limit) throw new ToolError(`edit: ${file_path} is ${st.size} bytes, exceeds ${limit}-byte limit. Use bash (sed/awk) to edit a large file.`);
				data = await fs$2.readFile(abs, "utf8");
			} catch (e) {
				if (e instanceof ToolError) throw e;
...
function betaGlobTool(ctx) {
	return betaTool({
		...
		run: async ({ pattern, path: searchPath }) => {
			...
			} else if (searchPath) root = await resolvePath(ctx, searchPath);
			if (!ctx.unrestrictedPaths && pat.split(/[\\/]/).includes("..")) throw new ToolError("glob: \"..\" is not permitted in the pattern");
			const realRoot = ctx.unrestrictedPaths ? root : await fs$2.realpath(root).catch(() => root);
			const matches = [];
			try {
				for await (const entry of fsGlob(pat, {
					...
					let mtime = 0;
					try {
						mtime = (await fs$2.stat(full)).mtimeMs;
					} catch {}
					matches.push({
						path: full,
						mtime
...
async function runWalkGrep(pattern, root, signal) {
	...
	if ((await fs$2.stat(root).catch(() => null))?.isFile()) await grepFile(root, re, push);
	else await walk(root, "", (rel) => grepFile(path$1.join(root, rel), re, push), signal);
...
async function walk(root, rel, fn, signal) {
	...
	async function inner(rel, depth) {
		...
		let entries;
		try {
			entries = await fs$2.readdir(path$1.join(root, rel), { withFileTypes: true });
		} catch {
			return true;
		}
		for (const e of entries) {
			...
			if (signal?.aborted) return false;
			const childRel = rel ? path$1.join(rel, e.name) : e.name;
			if (e.isDirectory()) {
...
async function findRg() {
	...
	for (const d of dirs) {
		...
		try {
			await fs$2.access(candidate, fssync.constants.X_OK);
			return candidate;
		} catch {}
	}
	return null;
...
//#endregion
export { Anthropic as n, transformJSONSchema as t };
